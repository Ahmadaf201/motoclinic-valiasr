...
create index if not exists idx_motorcycles_customer
on motorcycles(customer_id);

create index if not exists idx_cases_status
on service_cases(status);
...
select
 c.id,
 m.id,
 'سرویس اولیه و بررسی کامل',
 'پرونده نمونه برای تست سیستم'
from customers c
join motorcycles m
...
and not exists (
 ...
 where s.complaint = 'سرویس اولیه و بررسی کامل'
);
`;

async function migrate() {
  try {
    console.log("Starting MotoClinic database migration...");
